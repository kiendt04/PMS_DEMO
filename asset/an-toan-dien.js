...
(function () {
  ...
  function buildHeaders(q) {
    ...
    // Level 2: fixed null + per month: 'Số ngày công thưởng ATĐ'(1) + 'Hệ số đánh giá'(3)
    const h2 = [null, null, null];
    months.forEach(() => {
      h2.push({ label: 'Số ngày công thưởng ATĐ', colspan: 1 });
      h2.push({ label: 'Hệ số đánh giá', colspan: 3 });
    });

    // Level 3: fixed null + per month: null (for ngay cong) + Kcvi, Knqi, Ki
    const h3 = [null, null, null];
    months.forEach(() => {
      h3.push(null);     // merge with h2
      h3.push('Kcvi');
      h3.push('Knqi');
      h3.push('Ki');
    });
